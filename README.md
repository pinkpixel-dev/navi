...
- **Rich responses by default.** Assistant messages use restricted visual formatting for every answer unless you turn Rich Responses off in Settings. Navi keeps rich output behind `Thinking...` until the provider finishes. If a provider returns Markdown instead, Navi shows a format error instead of silently accepting it. The format supports cards, callouts, columns, steps, metrics, badges, and captions. Scripts, styles, forms, embedded content, SVG, remote resources, and unsafe links are not allowed inline. When the setting is off, assistant messages use sanitized Markdown.
...
- **An artifact canvas.** Navi asks every provider to put markdown, code, HTML, SVG, and Mermaid artifacts in fenced blocks. Complete unfenced HTML documents and standalone SVGs are recovered when a model misses that instruction. Artifacts open in a split-view canvas with rendered previews, syntax-highlighted code/raw views, and revision history. Download one artifact as a file, or grab everything as a zip.
