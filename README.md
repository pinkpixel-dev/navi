...
- **Richly formatted answers.** Assistant replies stream as sanitized Markdown. Navi turns structured headings into a clear introduction and stacked section cards, with responsive tables, callouts, lists, links, and highlighted code. This presentation is always active and does not require a setting or a custom provider format.
...
- **An artifact canvas.** Navi asks every provider to put code, HTML, SVG, Mermaid, and requested Markdown documents in fenced blocks. Ordinary chat Markdown stays unfenced so Navi can present it as response cards. Complete unfenced HTML documents and standalone SVGs are recovered when a model misses that instruction. Artifacts open in a split-view canvas with rendered previews, syntax-highlighted code/raw views, and revision history. Download one artifact as a file, or grab everything as a zip.
...
- **Run models locally, no extra installs.** Import a `.gguf` file from disk, hit Start, and Navi downloads a `llama-server` build matching your acceleration setting on first use (with a confirm prompt so it never happens silently), spawns it, and routes chat through it. Navi remembers which build it installed and tells you in Settings when llama.cpp publishes a newer one, which matters because new model architectures usually need a recent build. Updates download only when you click Update, and the previous build stays on disk in case you need to fall back. If you already have `llama-server` installed somewhere, you can point Settings at it instead and skip both the download and the update tracking.
...
- **Organize and personalize your chats.** Delete, rename, pin, archive, or move conversations from compact action menus in the sidebar or open chat header. Empty New Chat drafts stay out of the sidebar until you send a message. Projects live in their own sidebar section, have customizable icons/colors and instructions, and can be deleted without deleting their chats. Settings also supports dark/light theme mode, accent colors, custom avatars, a saved display name, a short bio, and custom instructions that get added to model context.
